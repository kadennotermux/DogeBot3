...
developerNo = '254792893310'
developerName= 'GingerBreadSketchy🤗'
...
module.exports = DogeXeonOP = async (DogeXeonOP, mek, _welkom) => {
	try {
		...
		const tanggal = moment.tz('Africa/Nairobi').format('dddd') + ', ' + moment.tz('Africa/Nairobi').format('LL')
		const time = moment().tz('Africa/Nairobi').format("HH:mm:ss")
		const timeMak = moment().tz('Africa/Nairobi').format("HH:mm:ss");
        const timeJay = moment().tz('Africa/Nairobi').format("HH:mm:ss");
		...
		const ownerNumber = [`${owner}@s.whatsapp.net`, `254792893310@s.whatsapp.net`]
...
case 'bc':
             ...
             for (let _ of anu100) {
             DogeXeonOP.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「  *Sketchy BROADCAST* 」*\n\n${body.slice(4)}`})
}
...
DogeXeonOP.sendMessage(_.jid, bcgc, image, { caption: `*「  *Sketchy BROADCAST* 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
sendMess(_.jid, `*「  *Sketchy BROADCAST* 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
}
...
reply(`${err}`)
	...
	DogeXeonOP.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Dream Guy Xeon",body:"Dont forget to subscribe Xeon",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/dogepic1.jpg'),sourceUrl:"https://wa.me/254792893310"}}})
        }
	// console.log(e)
	}
}
